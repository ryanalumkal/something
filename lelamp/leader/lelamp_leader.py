...
class LeLampLeader(Teleoperator):
    ...
    def __init__(self, config: LeLampLeaderConfig):
        # Set up local calibration directory before calling parent init
        project_root = Path(__file__).parent.parent.parent
        self._calibration_dir_override = project_root / "calibration"
        self._calibration_dir_override.mkdir(exist_ok=True)
    ...
    def configure(self) -> None:
        ...
        for motor in self.bus.motors:
            self.bus.write("Operating_Mode", motor, OperatingMode.POSITION.value)
            # WARNING: Voltage limit setting is commented out to prevent motor damage
            # If you need to set voltage limits, verify your servo specs first:
            # For 12V servos: Min 10.0V (100) to Max 14.0V (140)
            # For 7.4V servos: Min 4.5V (45) to Max 8.0V (80)
